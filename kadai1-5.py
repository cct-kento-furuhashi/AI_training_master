...
"""
5個の列を使い、lightGBMの機能を使って一番良い組み合わせを求める。
更にパラメータ最適化を行う
"""
...
if __name__ == "__main__":
    ...
    for idx, (train_data, valid_data) in enumerate(iter_kfold(load_data, n_splits=N_SPLITS)):
        # パラメータ最適化
        params = optimize_param(train_data, valid_data, test_data, name=f"optimize_param{idx}")

        # 訓練
        model = train(train_data, valid_data, params=params, model_idx=model_idx, is_save=False)
        ...
        # 結果格納
        result: List[Union[float, int, str]] = use_columns.copy()
    ...
    result_df.to_csv(os.path.join("outputs", "All_Result_1-5.csv"), index=False)
