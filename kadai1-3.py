...
if __name__ == "__main__":
    ...
    # 組み合わせ数
    comb_num = comb(len(load_data.columns.values) - 1, USE_COLUMN_NUM, exact=True) * N_SPLITS
    ...
    for idx, (train_data, valid_data) in enumerate(iter_kfold(load_data, n_splits=N_SPLITS)):
        ...
        for use_columns in iter_columns(load_data, k=USE_COLUMN_NUM):
            print(f"\r{model_idx} / {comb_num}", end="")
            ...
            model_idx += 1
    print(f"\r{comb_num} / {comb_num}", end="")
    # 結果作成
    result_df = DataFrame(result_list, columns=result_columns)
    # 結果出力
    os.makedirs("outputs", exist_ok=True)
    result_df.to_csv(os.path.join("outputs", "All_Result_1-3.csv"), index=False)
